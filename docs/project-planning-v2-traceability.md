...
| 前端框架 | About/Product/News/Contact、全局搜索 | `AppHeader.vue`、`PublicInfoView.vue`、`/search/aggregate` | 已实现 |
| 产品列表 | 机型与类型交叉筛选、OEM 展示 | `AggregateSearchView.vue`、`PublicMachineBrandsController` | 已实现 |
| 产品详情 | 主图/详情图、参数、适配、同 MR.1 推荐、询盘 | `PublicProductView.vue`、Razor Detail、`GalleryApp.vue`、`InquiryApp.vue` | 已实现 |
