...
| 产品列表 | 响应式产品卡片网格、机型与类型交叉筛选、Air/Oil/Fuel/Hydraulic 快捷筛选、OEM3 主图回退、OEM 展示、MR.1 内部聚合不对外显示；`/search` 统一进入公开聚合页 | `AggregateSearchView.vue`、`router/index.ts`、`PublicMachineBrandsController` | 已实现 |
| 产品详情 | 主图/详情图、参数、适配、同组 OEM 推荐、质量/价格/MOQ/交期询盘引导；公开界面、JSON 契约与 SEO URL 均不暴露 MR.1 | `PublicProductDetailDto`、`PublicProductView.vue`、Razor Detail、`GalleryApp.vue`、`InquiryApp.vue` | 已实现 |
| 检索规则 | Meilisearch 模糊命中、OEM 双向、范围检索；公开页面只展示上架产品与客户字段 | `MeiliSearchProvider`、`AggregateSearch`、`IProductDetailService` | 已实现 |
