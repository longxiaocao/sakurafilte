...
async function saveDialog() {
  ...
  if (!v) { ElMessage.warning('OEM 3 不能为空'); return }
  if (v.length > 200) { ElMessage.warning('OEM 3 长度不能超过 200'); return }
  try {
    if (dialogMode.value === 'create') {
      await dictApi.oemNo3s.create(v, dialogForm.sortOrder); ElMessage.success('已新增')
...
async function softDelete(row: OemNo3Item) {
  try { await ElMessageBox.confirm(`确定删除 "${row.oemNo3}" 吗? (软删除)`, '确认', { type: 'warning' }) } catch { return }
  try { await dictApi.oemNo3s.delete(row.id); ElMessage.success('已删除'); await load() }
  catch (e: any) { ElMessage.error(e?.response?.data?.detail || e?.message || '删除失败') }
}
async function restore(row: OemNo3Item) {
  try { await dictApi.oemNo3s.restore(row.id); ElMessage.success('已恢复'); await load() }
...
async function onDrop(e: DragEvent, targetId: number) {
  ...
  try { await dictApi.oemNo3s.reorder(updates); ElMessage.success('排序已保存') }
...
<template>
  <div class="p-3 max-w-screen-xl mx-auto">
    <div class="flex items-center gap-2 mb-3 flex-wrap">
      ...
      <div class="flex-1" />
      <el-input v-model="searchKw" placeholder="搜索 OEM 3" clearable size="small" style="width: 200px" @keyup.enter="onSearch" />
    ...
    <div class="hairline" v-loading="loading">
      ...
      <div v-if="!loading && items.length === 0" class="dict-empty">暂无数据, 点击右上"新增 OEM 3"开始</div>
    </div>

    <div class="mt-2 text-xs text-muted">共 {{ total }} 条 (启用 {{ activeCount }}, 软删 {{ total - activeCount }}) · 拖动"≡"列重排</div>

    <el-dialog v-model="dialogOpen" :title="dialogMode === 'create' ? '新增 OEM 3' : '编辑 OEM 3'" width="480px">
      <el-form :model="dialogForm" label-width="100px" size="small">
        <el-form-item label="OEM 3" required>
          <el-input v-model="dialogForm.oemNo3" placeholder="例: 11427622448" maxlength="200" show-word-limit />
        </el-form-item>
