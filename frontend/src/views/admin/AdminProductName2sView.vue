...
const dialogForm = reactive<{ id?: number; productName2: string; sortOrder: number }>({
  productName2: '', sortOrder: 0
})
...
function openCreate() {
  dialogMode.value = 'create'; dialogForm.id = undefined; dialogForm.productName2 = ''
  const maxSort = items.value.filter((x) => !x.deletedAt).reduce((m, x) => Math.max(m, x.sortOrder), 0)
  dialogForm.sortOrder = maxSort + 10
  dialogOpen.value = true
}
function openEdit(row: ProductName2Item) {
  dialogMode.value = 'edit'; dialogForm.id = row.id; dialogForm.productName2 = row.productName2; dialogForm.sortOrder = row.sortOrder
  dialogOpen.value = true
}
...
function isDraggable(row: ProductName2Item) { return !row.deletedAt }
function rowClass(row: ProductName2Item): string {
  const c: string[] = []
...
<template>
  <div class="p-3 w-full">
    <div class="flex items-center gap-2 mb-3 flex-wrap">
      ...
      <div class="flex-1" />
      <el-input v-model="searchKw" :placeholder="t('admin.productname2sview.placeholder.search_product_name')" clearable size="small" table-layout="auto" style="width: 200px" @keyup.enter="onSearch" />
      ...
      <el-button type="primary" size="small" table-layout="auto" @click="openCreate">新增产品名 2</el-button>
    </div>
    ...
    <div class="hairline" v-loading="loading">
      ...
      <div v-for="row in items" :key="row.id"
        ...
        <div class="cell-name">{{ row.productName2 }}</div>
