...
const dialogForm = reactive<{ id?: number; productName1: string; sortOrder: number }>({
  productName1: '',
  sortOrder: 0
})
...
async function onDrop(e: DragEvent, targetId: number) {
  ...
  const updates: ProductName1ReorderItem[] = items.value.map((it, idx) => ({
    id: it.id, sortOrder: (idx + 1) * 10
  }))
...
function isDraggable(row: ProductName1Item) { return !row.deletedAt }
function rowClass(row: ProductName1Item): string {
