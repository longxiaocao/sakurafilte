...
- [2026-07-19] [v30 验证 / P1] smoke.spec.ts ETL 页 networkidle 与 SSE 长连接不兼容: tests/functional/smoke.spec.ts L74 用 waitUntil:'networkidle' + timeout 15000, 但 ETL 页通过 useEtlProgress composable 建立 fetch + ReadableStream SSE 长连接持续接收进度, networkidle 永远达不到 → 15s 超时失败。修复 ~5min: 改 waitUntil 为 'domcontentloaded' 或 'load' (SSE 不影响 DOM 加载完成)。非 v30 回归 (SSE 是 V24-F78/F79 引入), 但 v30 端到端验证暴露。Playwright smoke 14/15 通过, 仅此 1 个失败 | 触发文件: frontend/tests/functional/smoke.spec.ts L74
