...
REPLACEMENTS = plan['replacements']  # list of dicts
...
def modify_i18n(ts_path: Path, is_zh: bool) -> tuple:
    ...
    for r in REPLACEMENTS:
        old_full = r['old']      # 'admin.{view}.{ctx}.lXXX_'
        new_full = r['new']      # 'common.action.{sem}'
        zh_v = r['zh']
        en_v = r['en']
        short_key = old_full.split('.')[-1]
        target_value = zh_v if is_zh else en_v

        # 行匹配: 任意缩进 + key + : + 'value' + 逗号
    ...
    if 'action: {' not in text:
        ...
        if common_match:
            ...
            action_lines = [f'{action_indent}action: {{']
