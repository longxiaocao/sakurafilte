...
def main():
    ...
    parser.add_argument(
        ...
        help="从 JSON 加载查询集 (由 build_bench_queries.py 采样生成)。"
             "省略则用本文件硬编码 QUERIES —— 注意硬编码集是为旧数据契约 "
             "(OEM 号 P0272887 / 机型 PC200) 设计的, 在 gen_perf_data.py 生成的"
             "数据上实测 54% 查询 0 命中, 会把 P95 拉低成假数据。"
    )
    parser.add_argument(
        "--rounds", type=int, default=1,
        help="每个并发档重复跑几轮 (默认 1)。样本数 = 查询数 × 轮数; "
             "只跑 1 轮时约 50 个样本, P99 实际等于第 49 名, 统计意义很弱。"
             "正式压测建议 >= 10 轮。"
    )
    args = parser.parse_args()

    # 查询集: 外部文件优先, 否则回退到硬编码 (保持原有行为不变)
    if args.queries_file:
        _payload = json.loads(open(args.queries_file, encoding="utf-8").read())
        queries = [tuple(q) for q in _payload["queries"]]
        typeahead_queries = _payload.get("typeahead") or TYPEAHEAD_QUERIES
        queries_source = args.queries_file
    else:
        queries = list(QUERIES)
        typeahead_queries = list(TYPEAHEAD_QUERIES)
        queries_source = "内置硬编码 QUERIES"

    print("=" * 80)
    print("Day 10+ P1.4 Search 性能基准 (Task 5)")
    print(f"  BASE = {BASE}")
    print(f"  queries = {len(queries)} (8 类) x {args.rounds} 轮 = "
          f"{len(queries) * args.rounds} 样本/档")
    print(f"  queries source = {queries_source}")
    ...
    results = {
        "meta": {
            ...
            "rounds": args.rounds,
            "samples_per_concurrency": len(queries) * args.rounds,
    ...
    for cc in concurrency_list:
        print(f"\n  --- concurrency={cc} ---")
        latencies, errors = run_concurrent(queries * args.rounds, cc, f"c{cc}")
    ...
    if not args.skip_typeahead:
        ...
        typeahead_pool = typeahead_queries * max(7, args.rounds)
