...
echo "[2/7] 构建并重建 postgres + api (--build --force-recreate)..."
...
# 6. SQL 迁移 (V3 2026-08-25 上线审查: 迁移无自动通道修复 — 对比 schema_migrations 执行未应用迁移)
echo "[6/7] 应用未执行 SQL 迁移..."
if bash scripts/migrate.sh; then
    echo "    [OK] 迁移检查/应用完成"
else
    echo "    [WARN] 迁移执行失败 — 请人工检查 (bash scripts/migrate.sh)"
fi

# 7. 健康检查
echo "[7/7] backend 健康检查..."
sleep 5
...
echo "全部完成。如 max_connections 与健康检查均 OK, 部署生效。"
echo "提示: 生产数据库请配置每日自动备份 (bash scripts/backup-db.sh, 见脚本头注册计划任务)。"
