...
# 前提: 已创建 .env (参考 .env.prod.example; 连接串若被环境变量覆盖须含 Maximum Pool Size=120;Connection Idle Lifetime=60)
#
...
# 2. env 检查
if [ ! -f .env ]; then
    echo "[FAIL] 缺少 .env — 请 cp .env.prod.example .env 并填入真实密钥"
    exit 1
fi

# 3. 重建 PG + API (配置变更生效的关键)
echo "[2/6] 重建 postgres + api (--force-recreate)..."
docker compose -f docker-compose.prod.yml --env-file .env up -d --force-recreate postgres api || { echo "[FAIL] 重建失败"; exit 1; }

# 4. 其余服务常规更新 (web/meilisearch/minio/prometheus/grafana)
echo "[3/6] 其余服务常规更新..."
docker compose -f docker-compose.prod.yml --env-file .env up -d || { echo "[FAIL] 常规更新失败"; exit 1; }
...
sleep 5
if curl -sf http://localhost:5148/health/ready >/dev/null 2>&1; then
