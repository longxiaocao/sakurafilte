...
--      过滤的 admin 检索都会先对全量 100 万 products 做 Parallel Seq Scan +
--      external-merge Sort (排序集 16MB 落盘), 再嵌套循环探测 machine_applications。
--      实测: machineBrand=CUMMINS 这类高命中量过滤 → SQL 1481ms / API ~1.7s。
...
-- IF NOT EXISTS: 重复执行安全 (本机 perf 环境已手动建过同名索引, 不会冲突)。
