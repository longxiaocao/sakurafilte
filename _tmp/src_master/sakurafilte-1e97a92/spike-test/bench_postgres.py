"""
Day 2: PostgreSQL 100 万数据性能基线
- 建表（含按 Type 分区 + 软删除字段 + history 表）
- 批量导入 100 万 products + 12.5M xrefs + 15.5M apps
- 跑查询性能: 模糊搜索 / ±5mm / 组合过滤
- 输出 P50/P95/P99 延迟报告
"""
import psycopg2
...
def print_result(name, r):
    print(f"  [{name:30s}] P50={r['p50_ms']:7.1f}ms | P95={r['p95_ms']:7.1f}ms | P99={r['p99_ms']:7.1f}ms | min={r['min_ms']:7.1f}ms | rows={r['rows']}")

print("=" * 80)
print("Day 2: PostgreSQL 100 万数据性能基线")
print("=" * 80)
...
if not cur.fetchone():
    ...
    print("[1] 创建数据库 spike_test")
else:
    print("[1] 数据库 spike_test 已存在")
...
# 2) 建表 (按 Type 分区)
print("\n[2] 建表 products (按 Type LIST 分区) + xrefs + apps + history")
ddl = """
...
# 3) COPY 导入 100 万 products
print("\n[3] 批量导入 products (100 万行) ...")
t0 = now()

import io

def copy_jsonl_to_table(jsonl_path, table_name, columns, transform=None):
    """流式 COPY: 读 jsonl → 写临时 CSV → COPY FROM"""
    buf = io.StringIO()
...
copy_jsonl_to_table(PROD, "products", product_cols)
prod_elapsed = now() - t0
print(f"    products: {prod_elapsed:.1f}s ({1_000_000/prod_elapsed:,.0f} rows/s)")

# 4) 导入 xrefs (12.5M) - 关联 product_id
print("\n[4] 批量导入 cross_references (12.5M) ...")
# 用 UNLOGGED TABLE 做映射(autocommit 模式下持久)
print("    [4a] 构建 OEM → product_id 映射 ...")
cur.execute("""
DROP TABLE IF EXISTS oem_to_id;
CREATE UNLOGGED TABLE oem_to_id (oem_no_normalized VARCHAR(50) PRIMARY KEY, product_id BIGINT NOT NULL);
INSERT INTO oem_to_id SELECT oem_no_normalized, id FROM products;
CREATE INDEX ON oem_to_id (product_id);
""")
cur.execute("SELECT COUNT(*) FROM oem_to_id")
print(f"    [4a] 映射构建完成 ({cur.fetchone()[0]:,} 行)")

t0 = now()
# 流式读 xref,关联 product_id,写入 cross_references
buf = io.StringIO()
xref_cols = ["product_id", "product_name_1", "oem_brand", "oem_no_3", "is_discontinued"]
# 临时缓存 product_id
cur.execute("CREATE TEMP TABLE oem_lookup (oem VARCHAR(50) PRIMARY KEY) ON COMMIT DROP")
cur.execute("INSERT INTO oem_lookup SELECT oem_no_normalized FROM products")
cur.execute("PREPARE get_pid (text) AS SELECT id FROM products WHERE oem_no_normalized = $1")
# 用 prepared + 批量
xref_count = 0
batch = []
BATCH = 5000
with open(XREF, encoding="utf-8") as f:
    for line in f:
        d = json.loads(line)
        cur.execute("SELECT id FROM products WHERE oem_no_normalized = %s", (d["product_oem"],))
        r = cur.fetchone()
        if not r: continue
        batch.append((r[0], d["product_name_1"], d["oem_brand"], d["oem_no_3"], False))
        if len(batch) >= BATCH:
            psycopg2.extras.execute_values(cur,
                "INSERT INTO cross_references (product_id, product_name_1, oem_brand, oem_no_3, is_discontinued) VALUES %s",
...
# 5) 导入 apps (15.5M)
print("\n[5] 批量导入 machine_applications (15.5M) ...")
t0 = now()
app_count = 0
batch = []
...
with open(APP, encoding="utf-8") as f:
    for line in f:
        ...
        r = cur.fetchone()
        if not r: continue
        # 解析 production_date_start
        ds = d.get("production_date_start_str", "")
        batch.append((r[0], d["machine_brand"], d["machine_model"], d["model_name"],
...
tests = {
    ...
    "2. 模糊搜索 - trgm 'SA' (前缀)":
        ("SELECT * FROM products WHERE oem_no_normalized ILIKE 'SA%' LIMIT 20", None),
    "3. 模糊搜索 - trgm 'SA42' (相似度)":
        ("SELECT * FROM products WHERE oem_no_normalized % 'SA42' ORDER BY similarity(oem_no_normalized, 'SA42') DESC LIMIT 20", None),
    "4. 精确 OEM (typed)":
        ("SELECT * FROM products WHERE oem_no_display = 'SA 42359'", None),
    "5. ±5mm 范围 D1":
        ("SELECT * FROM products WHERE d1_mm BETWEEN 173 AND 183 LIMIT 20", None),
    "6. ±5mm D1 + Type 过滤":
        ("SELECT * FROM products WHERE d1_mm BETWEEN 173 AND 183 AND type = 'AIR FILTER' LIMIT 20", None),
    "7. ±10mm D1 + D2 + H1 三维":
        ("SELECT * FROM products WHERE d1_mm BETWEEN 168 AND 188 AND d2_mm BETWEEN 130 AND 150 AND h1_mm BETWEEN 48 AND 68 LIMIT 20", None),
    "8. 模糊 OEM + ±5mm + Type 组合":
        ("SELECT * FROM products WHERE oem_no_normalized ILIKE 'SA%' AND d1_mm BETWEEN 173 AND 183 AND type = 'AIR FILTER' LIMIT 20", None),
    "9. 交叉引用 - 按 brand+no 查":
        ("SELECT p.* FROM products p JOIN cross_references x ON x.product_id = p.id WHERE x.oem_brand = 'BOSCH' AND x.oem_no_3 = '1234' LIMIT 20", None),
    "10. 机型适配 - 按 brand+model":
        ("SELECT p.* FROM products p JOIN machine_applications m ON m.product_id = p.id WHERE m.machine_brand = 'BMW' AND m.machine_model = 'B320' LIMIT 20", None),
    "11. 全表 COUNT":
        ("SELECT COUNT(*) FROM products", None),
    "12. 交叉引用 COUNT":
        ("SELECT COUNT(*) FROM cross_references", None),
    "13. 模糊 'A' 单字符 (最坏情况)":
        ("SELECT * FROM products WHERE oem_no_normalized ILIKE 'A%' LIMIT 20", None),
    "14. 软删除过滤 (默认)":
...
for name, r in results.items():
    ok = "✅" if r['p95_ms'] < 200 else "❌"
    report += f"| {name} | {r['p50_ms']} | {r['p95_ms']} | {r['p99_ms']} | {ok} |\n"

report += f"""
## 结论
- 100 万 products + 12.5M xrefs + 15.5M apps 总大小: {db_size}
- 模糊搜索 + 范围 + 组合过滤 P95 是否 < 200ms? 详见上表
- ETL 批量导入速度是否可接受? 详见导入性能表
"""

with open(OUT_DIR / "SPIKE-REPORT-pg.md", "w", encoding="utf-8") as f:
    f.write(report)

print(f"\n报告: {OUT_DIR}\\SPIKE-REPORT-pg.md")
print(f"\n=== Day 2 完成 ===\n")
